// frontend/app/(tourist)/experience-detail.js
...
export default function ExperienceDetailScreen() {
    ...
    const fetchHostAvailability = async () => {
        try {
            ...
            (res.data || []).forEach(booking => {
                // Optional: ignore cancelled bookings if your backend returns them
    ...
    const startRecording = async () => {
        try {
            // ✅ Prevent starting while processing or already listening
            ...
            // ✅ Auto-stop after 10 seconds (prevents stuck listening)
            clearAutoStopTimer();
    ...
    const stopRecording = async () => {
        ...
        try {
            ...
            if (uri) {
                // small delay so file becomes available
                setTimeout(() => {
    ...
    const handleVoiceQuery = async (uri) => {
        try {
            ...
            // ✅ IMPORTANT FIX: Do NOT strip file:// on iOS
            ...
            if (answer) {
                ...
                Speech.speak(answer, { rate: 0.9, pitch: 1.0 });
            } else {
                Alert.alert("No Answer", "Try asking in a different way.");
            }

        } catch (error) {
            console.log("Assistant error:", error?.response?.data || error?.message || error);
    ...
    const closeAssistantOverlay = () => {
        ...
        // If recording still exists, stop it
        if (recordingRef.current) {
            recordingRef.current.stopAndUnloadAsync().catch(() => {});
            recordingRef.current = null;
        }
    ...
    const handleConfirmBooking = async () => {
        ...
        if (disabledDates?.[selectedDate]?.disabled) {
            Alert.alert("Not Available", "The host is already booked on this date. Please choose another date.");
            return;
        }
        ...
        try {
            ...
            const bookingData = {
                ...
                bookingDate: finalBookingDate.toISOString(), // ✅ safer for backend
                guests: guestCount,
            ...
            >
                ...
                <View style={styles.contentBody}>
                    ...
                    <View style={styles.expertRow}>
                        <Image
                            source={{
                                uri: `https://ui-avatars.com/api/?background=2E7D32&color=fff&name=${exp?.hostName}`
                            }}
                            ...
                            <View style={styles.aiCardContent}>
                                <View style={styles.aiTextPart}>
                                    ...
                                    <Text style={styles.aiCardDesc}>
                                        Hold the button and ask about history or traditions.
                                    </Text>
                                    ...
                                    >
                                        ...
                                        <Text style={styles.historyLinkText}>View Q&A History</Text>
                                    </TouchableOpacity>
                                </View>
                            ...
                            {isListening ? (
                                <>
                                    ...
                                    <Text style={styles.voiceSubText}>
                                        Ask about Sri Lankan culture (auto stops in 10s)
                                    </Text>
                                </>
                            ) : isAiProcessing ? (
                                <>
                                ...
                                <View style={styles.fullScreenResult}>
                                    <View style={styles.transcriptSection}>
                                        ...
                                        <Text style={styles.transcriptText}>
                                            {recognizedQuestion ? `"${recognizedQuestion}"` : "Analyzing voice..."}
                                        </Text>
                                    </View>
                                    ...
                                    >
                                        <View style={styles.aiHeaderRow}>
                                            <MaterialCommunityIcons
                                                ...
                                                color="#FFA000"
                                            />
                                            <Text style={styles.aiHeaderLabel}>
                                                CeylonMate Guide
                                            </Text>
                                        </View>

                                        <Markdown style={fullMarkdownStyles}>
                                            {aiText}
                                        </Markdown>
                                        ...
                                        <Text style={styles.askAgainHint}>
                                            Hold the mic button again to ask another question
                                        </Text>
                                    </ScrollView>
                                </View>
                            )}
                    ...
                    <View style={styles.modalContent}>
                        ...
                        >
                                ...
                                <Calendar
                                    ...
                                    onDayPress={(day) => {
                                        if (disabledDates?.[day.dateString]?.disabled) {
                                            Alert.alert("Not Available", "Host is already booked on this date.");
                                            return;
                                        }
                            ...
                            <View style={styles.inputGrid}>
                                <View style={[styles.bookingCard, { flex: 1, marginRight: 8 }]}>
                                    <View style={styles.cardHeader}>
                                ...
                                <View style={[styles.bookingCard, { flex: 1, marginLeft: 8 }]}>
                                    <View style={styles.cardHeader}>
                            ...
                            <LinearGradient colors={['#2E7D32', '#1B5E20']} style={styles.mSummaryCard}>
                                <View>
                                    <Text style={styles.mTotalLabel}>Grand Total</Text>
                                    <Text style={styles.mTotalVal}>
                                        LKR {(exp?.price * guestCount).toLocaleString()}
                                    </Text>
                                </View>
                                <Ionicons name="receipt" size={30} color="rgba(255,255,255,0.2)" />
                            </LinearGradient>

                            <TouchableOpacity
                                style={[
                                    styles.mConfirmBtn,
                                    (!selectedDate || disabledDates?.[selectedDate]?.disabled) && { backgroundColor: '#EEE' }
                                ]}
                                onPress={handleConfirmBooking}
                                disabled={isBooking || !selectedDate || disabledDates?.[selectedDate]?.disabled}
                            >
                                {isBooking
