...
def _build_final_rank_score(
    ...
    cost = pd.to_numeric(df.get("avg_cost", 0.0), errors="coerce").fillna(
        _nanmed(df.get("avg_cost", pd.Series([0])), 2500)).values
    dur = pd.to_numeric(df.get("avg_duration_hours", 3.0), errors="coerce").fillna(
        _nanmed(df.get("avg_duration_hours", pd.Series([3])), 3.0)).values
...
def _route_nearest_neighbor(points_df: pd.DataFrame, start_lat: Optional[float],
                            start_lon: Optional[float]) -> pd.DataFrame:
...
def score_hotels_perfect(
    ...
    for _, h in df.iterrows():
        ...
        if center_lat is None or center_lon is None or np.isnan(center_lat) or np.isnan(center_lon) or np.isnan(
                lat) or np.isnan(lon):
            dists.append(np.nan)
        else:
...
def run_itinerary_inference(input_data: dict) -> dict:
    # accept both formats
    user = input_data.get("user_preferences", input_data)
    # --- normalize activity preferences ---
    raw_types = user.get("activity_types")

    if raw_types:
        if isinstance(raw_types, list):
            preferred_types = [str(x).lower().strip() for x in raw_types]
        else:
            preferred_types = [str(raw_types).lower().strip()]
    else:
        preferred_types = [str(user.get("activity_type", "general")).lower().strip()]
    ...
    for _, row in scored.iterrows():
        ...
        if category in preferred_types:
            score *= 1.18  # primary preference
        elif len(preferred_types) > 1:
            score *= 0.92  # mild penalty for non-preferred

        boosted_scores.append(score)

    scored["score"] = np.clip(boosted_scores, 0.0, 1.0)
    ...
    max_attractions = _safe_int(user.get("max_attractions", max_per_day * available_days),
                                max_per_day * available_days)
    ...
    total_budget = _safe_float(user.get("budget", tb.get("estimated_total_budget", 150000.0)),
                               tb.get("estimated_total_budget", 150000.0))
    ...
    cluster_radius = float(
        np.clip(_safe_float(user.get("cluster_radius_km", user.get("distance_preference", 80.0)), 80.0), 20.0,
                220.0))
    ...
    for _, row in routed.iterrows():
        ...
        if est_cost <= 0:
            est_cost = float(
                np.clip(_safe_float(user.get("default_attraction_cost_lkr", 2500.0), 2500.0), 500.0, 25000.0))
    ...
    if len(selected_df) > 0:
        ...
        day_plan = [{"day": i + 1, "items": [], "time_used_hours": 0.0, "visit_hours": 0.0, "travel_hours": 0.0,
                     "buffer_hours": fixed_daily_buffer, "time_used_hours_with_fixed_buffer": fixed_daily_buffer}
    ...
    result = {
        ...
        "itinerary_center": {
            "latitude": None if center_lat is None or (
                    isinstance(center_lat, float) and np.isnan(center_lat)) else float(center_lat),
            "longitude": None if center_lon is None or (
                    isinstance(center_lon, float) and np.isnan(center_lon)) else float(center_lon),
        },
...
def main():
    ...
    result = run_itinerary_inference(input_data)
